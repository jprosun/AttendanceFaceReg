...
class Lesson:
    def __init__(self,root):
        ...
        self.heading = Label(self.root, text=self.txt, font=("Times New Roman", 28, "bold"), bg="white", fg="#608def",
                             bd=5, relief=FLAT)
        ...
        #subject
        subjectLabel = Label(left_inside_frame, text="ID Môn học:", font=("times new roman", 12, "bold"), bg="white")
        subjectLabel.grid(row=6, column=0, padx=20, pady=10, sticky=W)

        # Combobox khởi tạo với values rỗng, sẽ fill sau khi chọn giảng viên
        self.subject_combo = ttk.Combobox(
            left_inside_frame,
            textvariable=self.var_subjectid,
            values=[],
            state="readonly",
            font=("times new roman", 12, "bold"),
            width=22
        )
        self.subject_combo.set("Chọn môn")
        self.subject_combo.grid(row=6, column=1, padx=20, pady=10, sticky=W)

        # BIND sự kiện khi chọn xong
        self.subject_combo.bind("<<ComboboxSelected>>", self.callSubject)
    ...
    def callback(self, *args):
        ...
        row = cursor.fetchone()
        self.var_teachername.set(row[0] if row else "")
        cursor.execute("SELECT Subject_id FROM teacher_has_subject WHERE Teacher_id=%s", (tid,))
        subject_ids = [r[0] for r in cursor.fetchall()]
        conn.close()
        self.subject_combo['values'] = subject_ids
        self.subject_combo.set(subject_ids[0] if subject_ids else 'Không có môn')
    def callSubject(self, *args):
        sid = self.var_subjectid.get()
        conn = mysql.connector.connect(host='localhost', user='root', password='', database='face_recognizer', port='3306')
        cursor = conn.cursor()
        cursor.execute("SELECT Subject_name FROM subject WHERE Subject_id=%s", (sid,))
        row = cursor.fetchone()
        conn.close()
        self.var_subjectname.set(row[0] if row else "")
    def getNextid(self):
        conn = mysql.connector.connect(host='localhost', user='root', password='', database='face_recognizer', port='3306')
        cursor = conn.cursor()
        cursor.execute("SELECT Lesson_id FROM lesson ORDER BY Lesson_id DESC LIMIT 1")
        lastid = cursor.fetchone()
        nextid = 1 if not lastid else int(lastid[0]) + 1
    ...
    def update_data(self):
        ...
        if self.var_id.get()=="" or self.var_timestart.get()=="" or self.var_timeend.get()=="" \
            ...
            if not messagebox.askyesno("Update", "Bạn có chắc muốn cập nhật?", parent=self.root):
                return
            ...
            try:
                ...
                print("Updated rows:", cursor.rowcount)  # <-- debug
                conn.commit()
                conn.close()
                ...
                messagebox.showerror("Lỗi", f"Do lỗi: {e}", parent=self.root)
